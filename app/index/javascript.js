function att(data){
	...
	for (var c=0;c<data.length;c++){
		if(data[c].status == 'I') options+='<option value='+data[c].chave+' style="color:red">'+data[c].nome+'</option>';
...
function show(allData,url){
	$.getJSON(url, function(data){
		...
		if(allData==true){
			...
			if(data.status == 'I') result+='<tr><td colspan="4" style="color:red">Este produto não está disponível</td></tr>';
		}
		$('#table').html(result);
...
$(document).ready(function(){
	$.getJSON('http://192.168.1.109:8080/list', function(data){
		...
		$('#selecionar').change(function(){
			...
			else if(i == 0) show(true, 'http://192.168.1.109:8080/list');
			else if(i != 0) show(false, 'http://192.168.1.109:8080/product?chave='+i);
		});
	});	
	/*
